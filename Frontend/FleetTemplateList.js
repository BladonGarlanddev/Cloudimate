...
const FleetTemplateList = ({fleetTemplates, handleCheck, setDetails, requestMade }) => {
    return ( 
        <tbody className="text-gray-600">
                {fleetTemplates && fleetTemplates.length > 0 ? (fleetTemplates.map((template) => (
                    <tr key={template.id} className="table-row-body">
                        ...
                        <td className="p-2 text-ellipsis">{template["fleet_type"]}</td>
...
export default FleetTemplateList;